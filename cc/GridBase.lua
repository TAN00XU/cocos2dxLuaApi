---@meta

---@diagnostic disable: lowercase-global, missing-return, unused-local

--- Cocos API 类：`cc.GridBase`。
--- 继承：`cc.Ref`。
---@class cc.GridBase : cc.Ref
local GridBase = {}
cc.GridBase = GridBase

--- 设置 `cc.GridBase:setGridSize` 对应的值。
---
--- 参数说明：
--- - `gridSize`：参数 `gridSize`，类型为 `size_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param gridSize size_table 参数 `gridSize`，类型为 `size_table`。
---@return self 当前对象，便于链式调用。
function GridBase:setGridSize(gridSize) end

--- 设置 `cc.GridBase:setGridRect` 对应的值。
---
--- 参数说明：
--- - `rect`：矩形区域。类型为 `rect_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param rect rect_table 矩形区域。类型为 `rect_table`。
---@return self 当前对象，便于链式调用。
function GridBase:setGridRect(rect) end

--- 调用 `cc.GridBase:calculateVertexPoints`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:calculateVertexPoints() end

--- 调用 `cc.GridBase:reuse`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:reuse() end

--- 调用 `cc.GridBase:beforeDraw`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:beforeDraw() end

--- 获取 `cc.GridBase:getGridRect` 对应的值。
---
--- 返回说明：
--- - `rect_table`：获取到的 Lua 表数据。
---@return rect_table 获取到的 Lua 表数据。
function GridBase:getGridRect() end

--- 判断 `cc.GridBase:isTextureFlipped` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function GridBase:isTextureFlipped() end

--- 获取 `cc.GridBase:getGridSize` 对应的值。
---
--- 返回说明：
--- - `size_table`：获取到的 Lua 表数据。
---@return size_table 获取到的 Lua 表数据。
function GridBase:getGridSize() end

--- 调用 `cc.GridBase:afterBlit`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:afterBlit() end

--- 调用 `cc.GridBase:set2DProjection`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:set2DProjection() end

--- 获取 `cc.GridBase:getStep` 对应的值。
---
--- 返回说明：
--- - `vec2_table`：获取到的 Lua 表数据。
---@return vec2_table 获取到的 Lua 表数据。
function GridBase:getStep() end

--- 设置 `cc.GridBase:setStep` 对应的值。
---
--- 参数说明：
--- - `step`：参数 `step`，类型为 `vec2_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param step vec2_table 参数 `step`，类型为 `vec2_table`。
---@return self 当前对象，便于链式调用。
function GridBase:setStep(step) end

--- 设置 `cc.GridBase:setTextureFlipped` 对应的值。
---
--- 参数说明：
--- - `flipped`：参数 `flipped`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param flipped boolean 参数 `flipped`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function GridBase:setTextureFlipped(flipped) end

--- 调用 `cc.GridBase:blit`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:blit() end

--- 设置 `cc.GridBase:setActive` 对应的值。
---
--- 参数说明：
--- - `active`：参数 `active`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param active boolean 参数 `active`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function GridBase:setActive(active) end

--- 获取 `cc.GridBase:getReuseGrid` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function GridBase:getReuseGrid() end

--- 初始化 `cc.GridBase:initWithSize` 对应的对象或状态。
---
--- 参数说明：
--- - `gridSize`：参数 `gridSize`，类型为 `size_table`。
--- - `texture`：纹理对象。类型为 `cc.Texture2D`。
--- - `flipped`：参数 `flipped`，类型为 `boolean`。
--- - `rect`：矩形区域。类型为 `rect_table`。
---
--- 返回说明：
--- - `boolean`：初始化是否成功。
---@overload fun(gridSize: size_table, texture: rect_table): boolean
---@overload fun(gridSize: size_table): boolean
---@overload fun(gridSize: size_table, texture: cc.Texture2D, flipped: boolean): boolean
---@overload fun(gridSize: size_table, texture: cc.Texture2D, flipped: boolean, rect: rect_table): boolean
---@param gridSize? size_table 参数 `gridSize`，类型为 `size_table`。
---@param texture? cc.Texture2D 纹理对象。类型为 `cc.Texture2D`。
---@param flipped? boolean 参数 `flipped`，类型为 `boolean`。
---@param rect? rect_table 矩形区域。类型为 `rect_table`。
---@return boolean 初始化是否成功。
function GridBase:initWithSize(gridSize, texture, flipped, rect) end

--- 调用 `cc.GridBase:beforeBlit`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function GridBase:beforeBlit() end

--- 设置 `cc.GridBase:setReuseGrid` 对应的值。
---
--- 参数说明：
--- - `reuseGrid`：参数 `reuseGrid`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param reuseGrid integer 参数 `reuseGrid`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function GridBase:setReuseGrid(reuseGrid) end

--- 判断 `cc.GridBase:isActive` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function GridBase:isActive() end

--- 调用 `cc.GridBase:afterDraw`。
---
--- 参数说明：
--- - `target`：目标对象。类型为 `cc.Node`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param target cc.Node 目标对象。类型为 `cc.Node`。
---@return self 当前对象，便于链式调用。
function GridBase:afterDraw(target) end
