---@meta

---@diagnostic disable: lowercase-global, missing-return, unused-local

--- Cocos API 类：`cc.Manifest`。
--- 继承：`cc.Ref`。
---@class cc.Manifest : cc.Ref
local Manifest = {}
cc.Manifest = Manifest

--- 获取 `cc.Manifest:getManifestFileUrl` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function Manifest:getManifestFileUrl() end

--- 判断 `cc.Manifest:isVersionLoaded` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function Manifest:isVersionLoaded() end

--- 判断 `cc.Manifest:isLoaded` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function Manifest:isLoaded() end

--- 获取 `cc.Manifest:getPackageUrl` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function Manifest:getPackageUrl() end

--- 获取 `cc.Manifest:getVersion` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function Manifest:getVersion() end

--- 获取 `cc.Manifest:getVersionFileUrl` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function Manifest:getVersionFileUrl() end

--- 获取 `cc.Manifest:getSearchPaths` 对应的值。
---
--- 返回说明：
--- - `array_table`：获取到的 Lua 表数据。
---@return array_table 获取到的 Lua 表数据。
function Manifest:getSearchPaths() end
