---@meta

---@diagnostic disable: lowercase-global, missing-return, unused-local

--- Cocos API 类：`ccui.LayoutComponent`。
--- 继承：`cc.Component`。
---@class ccui.LayoutComponent : cc.Component
local LayoutComponent = {}
ccui.LayoutComponent = LayoutComponent

--- 设置 `ccui.LayoutComponent:setStretchWidthEnabled` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setStretchWidthEnabled(isUsed) end

--- 设置 `ccui.LayoutComponent:setPercentWidth` 对应的值。
---
--- 参数说明：
--- - `percentWidth`：参数 `percentWidth`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param percentWidth number 参数 `percentWidth`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPercentWidth(percentWidth) end

--- 获取 `ccui.LayoutComponent:getAnchorPosition` 对应的值。
---
--- 返回说明：
--- - `vec2_table`：获取到的 Lua 表数据。
---@return vec2_table 获取到的 Lua 表数据。
function LayoutComponent:getAnchorPosition() end

--- 设置 `ccui.LayoutComponent:setPositionPercentXEnabled` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPositionPercentXEnabled(isUsed) end

--- 设置 `ccui.LayoutComponent:setStretchHeightEnabled` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setStretchHeightEnabled(isUsed) end

--- 设置 `ccui.LayoutComponent:setActiveEnabled` 对应的值。
---
--- 参数说明：
--- - `enable`：是否启用。类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param enable boolean 是否启用。类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setActiveEnabled(enable) end

--- 获取 `ccui.LayoutComponent:getRightMargin` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getRightMargin() end

--- 获取 `ccui.LayoutComponent:getSize` 对应的值。
---
--- 返回说明：
--- - `size_table`：获取到的 Lua 表数据。
---@return size_table 获取到的 Lua 表数据。
function LayoutComponent:getSize() end

--- 设置 `ccui.LayoutComponent:setAnchorPosition` 对应的值。
---
--- 参数说明：
--- - `point`：参数 `point`，类型为 `vec2_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param point vec2_table 参数 `point`，类型为 `vec2_table`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setAnchorPosition(point) end

--- 调用 `ccui.LayoutComponent:refreshLayout`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function LayoutComponent:refreshLayout() end

--- 判断 `ccui.LayoutComponent:isPercentWidthEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function LayoutComponent:isPercentWidthEnabled() end

--- 设置 `ccui.LayoutComponent:setVerticalEdge` 对应的值。
---
--- 参数说明：
--- - `vEage`：参数 `vEage`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param vEage integer 参数 `vEage`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setVerticalEdge(vEage) end

--- 获取 `ccui.LayoutComponent:getTopMargin` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getTopMargin() end

--- 设置 `ccui.LayoutComponent:setSizeWidth` 对应的值。
---
--- 参数说明：
--- - `width`：宽度。类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param width number 宽度。类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setSizeWidth(width) end

--- 获取 `ccui.LayoutComponent:getPercentContentSize` 对应的值。
---
--- 返回说明：
--- - `vec2_table`：获取到的 Lua 表数据。
---@return vec2_table 获取到的 Lua 表数据。
function LayoutComponent:getPercentContentSize() end

--- 获取 `ccui.LayoutComponent:getVerticalEdge` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function LayoutComponent:getVerticalEdge() end

--- 设置 `ccui.LayoutComponent:setPercentWidthEnabled` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPercentWidthEnabled(isUsed) end

--- 判断 `ccui.LayoutComponent:isStretchWidthEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function LayoutComponent:isStretchWidthEnabled() end

--- 设置 `ccui.LayoutComponent:setLeftMargin` 对应的值。
---
--- 参数说明：
--- - `margin`：参数 `margin`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param margin number 参数 `margin`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setLeftMargin(margin) end

--- 获取 `ccui.LayoutComponent:getSizeWidth` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getSizeWidth() end

--- 设置 `ccui.LayoutComponent:setPositionPercentYEnabled` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPositionPercentYEnabled(isUsed) end

--- 获取 `ccui.LayoutComponent:getSizeHeight` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getSizeHeight() end

--- 获取 `ccui.LayoutComponent:getPositionPercentY` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getPositionPercentY() end

--- 获取 `ccui.LayoutComponent:getPositionPercentX` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getPositionPercentX() end

--- 设置 `ccui.LayoutComponent:setTopMargin` 对应的值。
---
--- 参数说明：
--- - `margin`：参数 `margin`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param margin number 参数 `margin`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setTopMargin(margin) end

--- 获取 `ccui.LayoutComponent:getPercentHeight` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getPercentHeight() end

--- 获取 `ccui.LayoutComponent:getUsingPercentContentSize` 对应的值。
---
--- 返回说明：
--- - `boolean`：获取到的 布尔值。
---@return boolean 获取到的 布尔值。
function LayoutComponent:getUsingPercentContentSize() end

--- 设置 `ccui.LayoutComponent:setPositionPercentY` 对应的值。
---
--- 参数说明：
--- - `percentMargin`：参数 `percentMargin`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param percentMargin number 参数 `percentMargin`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPositionPercentY(percentMargin) end

--- 设置 `ccui.LayoutComponent:setPositionPercentX` 对应的值。
---
--- 参数说明：
--- - `percentMargin`：参数 `percentMargin`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param percentMargin number 参数 `percentMargin`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPositionPercentX(percentMargin) end

--- 设置 `ccui.LayoutComponent:setRightMargin` 对应的值。
---
--- 参数说明：
--- - `margin`：参数 `margin`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param margin number 参数 `margin`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setRightMargin(margin) end

--- 判断 `ccui.LayoutComponent:isPositionPercentYEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function LayoutComponent:isPositionPercentYEnabled() end

--- 设置 `ccui.LayoutComponent:setPercentHeight` 对应的值。
---
--- 参数说明：
--- - `percentHeight`：参数 `percentHeight`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param percentHeight number 参数 `percentHeight`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPercentHeight(percentHeight) end

--- 设置 `ccui.LayoutComponent:setPercentOnlyEnabled` 对应的值。
---
--- 参数说明：
--- - `enable`：是否启用。类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param enable boolean 是否启用。类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPercentOnlyEnabled(enable) end

--- 设置 `ccui.LayoutComponent:setHorizontalEdge` 对应的值。
---
--- 参数说明：
--- - `hEage`：参数 `hEage`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param hEage integer 参数 `hEage`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setHorizontalEdge(hEage) end

--- 设置 `ccui.LayoutComponent:setPosition` 对应的值。
---
--- 参数说明：
--- - `position`：位置坐标。类型为 `vec2_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param position vec2_table 位置坐标。类型为 `vec2_table`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPosition(position) end

--- 设置 `ccui.LayoutComponent:setUsingPercentContentSize` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setUsingPercentContentSize(isUsed) end

--- 获取 `ccui.LayoutComponent:getLeftMargin` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getLeftMargin() end

--- 获取 `ccui.LayoutComponent:getPosition` 对应的值。
---
--- 返回说明：
--- - `vec2_table`：获取到的 Lua 表数据。
---@return vec2_table 获取到的 Lua 表数据。
function LayoutComponent:getPosition() end

--- 设置 `ccui.LayoutComponent:setSizeHeight` 对应的值。
---
--- 参数说明：
--- - `height`：高度。类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param height number 高度。类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setSizeHeight(height) end

--- 判断 `ccui.LayoutComponent:isPositionPercentXEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function LayoutComponent:isPositionPercentXEnabled() end

--- 获取 `ccui.LayoutComponent:getBottomMargin` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getBottomMargin() end

--- 设置 `ccui.LayoutComponent:setPercentHeightEnabled` 对应的值。
---
--- 参数说明：
--- - `isUsed`：参数 `isUsed`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param isUsed boolean 参数 `isUsed`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPercentHeightEnabled(isUsed) end

--- 设置 `ccui.LayoutComponent:setPercentContentSize` 对应的值。
---
--- 参数说明：
--- - `percent`：百分比数值。类型为 `vec2_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param percent vec2_table 百分比数值。类型为 `vec2_table`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setPercentContentSize(percent) end

--- 判断 `ccui.LayoutComponent:isPercentHeightEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function LayoutComponent:isPercentHeightEnabled() end

--- 获取 `ccui.LayoutComponent:getPercentWidth` 对应的值。
---
--- 返回说明：
--- - `number`：获取到的 数值。
---@return number 获取到的 数值。
function LayoutComponent:getPercentWidth() end

--- 获取 `ccui.LayoutComponent:getHorizontalEdge` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function LayoutComponent:getHorizontalEdge() end

--- 判断 `ccui.LayoutComponent:isStretchHeightEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function LayoutComponent:isStretchHeightEnabled() end

--- 设置 `ccui.LayoutComponent:setBottomMargin` 对应的值。
---
--- 参数说明：
--- - `margin`：参数 `margin`，类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param margin number 参数 `margin`，类型为 `number`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setBottomMargin(margin) end

--- 设置 `ccui.LayoutComponent:setSize` 对应的值。
---
--- 参数说明：
--- - `size`：尺寸。类型为 `size_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param size size_table 尺寸。类型为 `size_table`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:setSize(size) end

--- 创建 `ccui.LayoutComponent:create` 对应的对象。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function LayoutComponent:create() end

--- 调用 `ccui.LayoutComponent:bindLayoutComponent`。
---
--- 参数说明：
--- - `node`：节点对象。类型为 `cc.Node`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param node cc.Node 节点对象。类型为 `cc.Node`。
---@return self 当前对象，便于链式调用。
function LayoutComponent:bindLayoutComponent(node) end

--- 初始化 `ccui.LayoutComponent:init` 对应的对象或状态。
---
--- 返回说明：
--- - `boolean`：初始化是否成功。
---@return boolean 初始化是否成功。
function LayoutComponent:init() end

--- 调用 `ccui.LayoutComponent:LayoutComponent`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function LayoutComponent:LayoutComponent() end
