---@meta

---@diagnostic disable: lowercase-global, missing-return, unused-local

--- Cocos API 类：`ccui.TextField`。
--- 继承：`ccui.Widget`。
---@class ccui.TextField : ccui.Widget
local TextField = {}
ccui.TextField = TextField

--- 设置 `ccui.TextField:setAttachWithIME` 对应的值。
---
--- 参数说明：
--- - `attach`：参数 `attach`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param attach boolean 参数 `attach`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setAttachWithIME(attach) end

--- 获取 `ccui.TextField:getFontSize` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function TextField:getFontSize() end

--- 获取 `ccui.TextField:getString` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function TextField:getString() end

--- 设置 `ccui.TextField:setPasswordStyleText` 对应的值。
---
--- 参数说明：
--- - `styleText`：参数 `styleText`，类型为 `string`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param styleText string 参数 `styleText`，类型为 `string`。
---@return self 当前对象，便于链式调用。
function TextField:setPasswordStyleText(styleText) end

--- 获取 `ccui.TextField:getDeleteBackward` 对应的值。
---
--- 返回说明：
--- - `boolean`：获取到的 布尔值。
---@return boolean 获取到的 布尔值。
function TextField:getDeleteBackward() end

--- 获取 `ccui.TextField:getTextColor` 对应的值。
---
--- 返回说明：
--- - `color4b_table`：获取到的 Lua 表数据。
---@return color4b_table 获取到的 Lua 表数据。
function TextField:getTextColor() end

--- 获取 `ccui.TextField:getPlaceHolder` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function TextField:getPlaceHolder() end

--- 获取 `ccui.TextField:getAttachWithIME` 对应的值。
---
--- 返回说明：
--- - `boolean`：获取到的 布尔值。
---@return boolean 获取到的 布尔值。
function TextField:getAttachWithIME() end

--- 设置 `ccui.TextField:setFontName` 对应的值。
---
--- 参数说明：
--- - `name`：名称或标识。类型为 `string`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param name string 名称或标识。类型为 `string`。
---@return self 当前对象，便于链式调用。
function TextField:setFontName(name) end

--- 获取 `ccui.TextField:getInsertText` 对应的值。
---
--- 返回说明：
--- - `boolean`：获取到的 布尔值。
---@return boolean 获取到的 布尔值。
function TextField:getInsertText() end

--- 设置 `ccui.TextField:setInsertText` 对应的值。
---
--- 参数说明：
--- - `insertText`：参数 `insertText`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param insertText boolean 参数 `insertText`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setInsertText(insertText) end

--- 设置 `ccui.TextField:setString` 对应的值。
---
--- 参数说明：
--- - `text`：文本内容。类型为 `string`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param text string 文本内容。类型为 `string`。
---@return self 当前对象，便于链式调用。
function TextField:setString(text) end

--- 获取 `ccui.TextField:getDetachWithIME` 对应的值。
---
--- 返回说明：
--- - `boolean`：获取到的 布尔值。
---@return boolean 获取到的 布尔值。
function TextField:getDetachWithIME() end

--- 设置 `ccui.TextField:setTextVerticalAlignment` 对应的值。
---
--- 参数说明：
--- - `alignment`：参数 `alignment`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param alignment integer 参数 `alignment`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function TextField:setTextVerticalAlignment(alignment) end

--- 添加 `ccui.TextField:addEventListener` 对应的对象或数据。
---
--- 参数说明：
--- - `callback`：回调函数。类型为 `fun(...): any`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param callback fun(...): any 回调函数。类型为 `fun(...): any`。
---@return self 当前对象，便于链式调用。
function TextField:addEventListener(callback) end

--- 调用 `ccui.TextField:didNotSelectSelf`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function TextField:didNotSelectSelf() end

--- 获取 `ccui.TextField:getFontName` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function TextField:getFontName() end

--- 设置 `ccui.TextField:setTextAreaSize` 对应的值。
---
--- 参数说明：
--- - `size`：尺寸。类型为 `size_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param size size_table 尺寸。类型为 `size_table`。
---@return self 当前对象，便于链式调用。
function TextField:setTextAreaSize(size) end

--- 调用 `ccui.TextField:attachWithIME`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function TextField:attachWithIME() end

--- 获取 `ccui.TextField:getStringLength` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function TextField:getStringLength() end

--- 获取 `ccui.TextField:getAutoRenderSize` 对应的值。
---
--- 返回说明：
--- - `size_table`：获取到的 Lua 表数据。
---@return size_table 获取到的 Lua 表数据。
function TextField:getAutoRenderSize() end

--- 设置 `ccui.TextField:setPasswordEnabled` 对应的值。
---
--- 参数说明：
--- - `enable`：是否启用。类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param enable boolean 是否启用。类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setPasswordEnabled(enable) end

--- 获取 `ccui.TextField:getPlaceHolderColor` 对应的值。
---
--- 返回说明：
--- - `color4b_table`：获取到的 Lua 表数据。
---@return color4b_table 获取到的 Lua 表数据。
function TextField:getPlaceHolderColor() end

--- 获取 `ccui.TextField:getPasswordStyleText` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function TextField:getPasswordStyleText() end

--- 设置 `ccui.TextField:setMaxLengthEnabled` 对应的值。
---
--- 参数说明：
--- - `enable`：是否启用。类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param enable boolean 是否启用。类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setMaxLengthEnabled(enable) end

--- 判断 `ccui.TextField:isPasswordEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function TextField:isPasswordEnabled() end

--- 设置 `ccui.TextField:setDeleteBackward` 对应的值。
---
--- 参数说明：
--- - `deleteBackward`：参数 `deleteBackward`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param deleteBackward boolean 参数 `deleteBackward`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setDeleteBackward(deleteBackward) end

--- 设置 `ccui.TextField:setCursorPosition` 对应的值。
---
--- 参数说明：
--- - `cursorPosition`：参数 `cursorPosition`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param cursorPosition integer 参数 `cursorPosition`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function TextField:setCursorPosition(cursorPosition) end

--- 获取 `ccui.TextField:getTextHorizontalAlignment` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function TextField:getTextHorizontalAlignment() end

--- 设置 `ccui.TextField:setFontSize` 对应的值。
---
--- 参数说明：
--- - `size`：尺寸。类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param size integer 尺寸。类型为 `integer`。
---@return self 当前对象，便于链式调用。
function TextField:setFontSize(size) end

--- 设置 `ccui.TextField:setPlaceHolder` 对应的值。
---
--- 参数说明：
--- - `value`：数值或对象值。类型为 `string`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param value string 数值或对象值。类型为 `string`。
---@return self 当前对象，便于链式调用。
function TextField:setPlaceHolder(value) end

--- 设置 `ccui.TextField:setCursorFromPoint` 对应的值。
---
--- 参数说明：
--- - `point`：参数 `point`，类型为 `vec2_table`。
--- - `camera`：参数 `camera`，类型为 `cc.Camera`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param point vec2_table 参数 `point`，类型为 `vec2_table`。
---@param camera cc.Camera 参数 `camera`，类型为 `cc.Camera`。
---@return self 当前对象，便于链式调用。
function TextField:setCursorFromPoint(point, camera) end

--- 设置 `ccui.TextField:setPlaceHolderColor` 对应的值。
---
--- 参数说明：
--- - `color`：颜色值。类型为 `color3b_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@overload fun(color: color4b_table): self
---@overload fun(color: color3b_table): self
---@param color color3b_table 颜色值。类型为 `color3b_table`。
---@return self 当前对象，便于链式调用。
function TextField:setPlaceHolderColor(color) end

--- 设置 `ccui.TextField:setTextHorizontalAlignment` 对应的值。
---
--- 参数说明：
--- - `alignment`：参数 `alignment`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param alignment integer 参数 `alignment`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function TextField:setTextHorizontalAlignment(alignment) end

--- 设置 `ccui.TextField:setTextColor` 对应的值。
---
--- 参数说明：
--- - `textColor`：参数 `textColor`，类型为 `color4b_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param textColor color4b_table 参数 `textColor`，类型为 `color4b_table`。
---@return self 当前对象，便于链式调用。
function TextField:setTextColor(textColor) end

--- 设置 `ccui.TextField:setCursorChar` 对应的值。
---
--- 参数说明：
--- - `cursor`：参数 `cursor`，类型为 `string`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param cursor string 参数 `cursor`，类型为 `string`。
---@return self 当前对象，便于链式调用。
function TextField:setCursorChar(cursor) end

--- 获取 `ccui.TextField:getMaxLength` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function TextField:getMaxLength() end

--- 判断 `ccui.TextField:isMaxLengthEnabled` 对应状态是否成立。
---
--- 返回说明：
--- - `boolean`：状态判断结果。
---@return boolean 状态判断结果。
function TextField:isMaxLengthEnabled() end

--- 设置 `ccui.TextField:setDetachWithIME` 对应的值。
---
--- 参数说明：
--- - `detach`：参数 `detach`，类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param detach boolean 参数 `detach`，类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setDetachWithIME(detach) end

--- 获取 `ccui.TextField:getTextVerticalAlignment` 对应的值。
---
--- 返回说明：
--- - `integer`：获取到的 整数值。
---@return integer 获取到的 整数值。
function TextField:getTextVerticalAlignment() end

--- 设置 `ccui.TextField:setTouchAreaEnabled` 对应的值。
---
--- 参数说明：
--- - `enable`：是否启用。类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param enable boolean 是否启用。类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setTouchAreaEnabled(enable) end

--- 设置 `ccui.TextField:setMaxLength` 对应的值。
---
--- 参数说明：
--- - `length`：参数 `length`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param length integer 参数 `length`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function TextField:setMaxLength(length) end

--- 设置 `ccui.TextField:setCursorEnabled` 对应的值。
---
--- 参数说明：
--- - `enabled`：是否启用。类型为 `boolean`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param enabled boolean 是否启用。类型为 `boolean`。
---@return self 当前对象，便于链式调用。
function TextField:setCursorEnabled(enabled) end

--- 设置 `ccui.TextField:setTouchSize` 对应的值。
---
--- 参数说明：
--- - `size`：尺寸。类型为 `size_table`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param size size_table 尺寸。类型为 `size_table`。
---@return self 当前对象，便于链式调用。
function TextField:setTouchSize(size) end

--- 获取 `ccui.TextField:getTouchSize` 对应的值。
---
--- 返回说明：
--- - `size_table`：获取到的 Lua 表数据。
---@return size_table 获取到的 Lua 表数据。
function TextField:getTouchSize() end

--- 创建 `ccui.TextField:create` 对应的对象。
---
--- 参数说明：
--- - `placeholder`：参数 `placeholder`，类型为 `string`。
--- - `fontName`：字体名称。类型为 `string`。
--- - `fontSize`：参数 `fontSize`，类型为 `integer`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@overload fun(placeholder: string, fontName: string, fontSize: integer): self
---@overload fun(): self
---@param placeholder? string 参数 `placeholder`，类型为 `string`。
---@param fontName? string 字体名称。类型为 `string`。
---@param fontSize? integer 参数 `fontSize`，类型为 `integer`。
---@return self 当前对象，便于链式调用。
function TextField:create(placeholder, fontName, fontSize) end

--- 创建 `ccui.TextField:createInstance` 对应的对象。
---
--- 返回说明：
--- - `cc.Ref`：创建出的 `cc.Ref` 对象或值。
---@return cc.Ref 创建出的 `cc.Ref` 对象或值。
function TextField:createInstance() end

--- 获取 `ccui.TextField:getVirtualRenderer` 对应的值。
---
--- 返回说明：
--- - `cc.Node`：获取到的 `cc.Node` 对象或值。
---@return cc.Node 获取到的 `cc.Node` 对象或值。
function TextField:getVirtualRenderer() end

--- 获取 `ccui.TextField:getDescription` 对应的值。
---
--- 返回说明：
--- - `string`：获取到的 字符串。
---@return string 获取到的 字符串。
function TextField:getDescription() end

--- 更新 `ccui.TextField:update` 对应的状态。
---
--- 参数说明：
--- - `dt`：帧间隔时间。类型为 `number`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@param dt number 帧间隔时间。类型为 `number`。
---@return self 当前对象，便于链式调用。
function TextField:update(dt) end

--- 调用 `ccui.TextField:hitTest`。
---
--- 参数说明：
--- - `pt`：参数 `pt`，类型为 `vec2_table`。
--- - `camera`：参数 `camera`，类型为 `cc.Camera`。
--- - `p`：参数 `p`，类型为 `vec3_table`。
---
--- 返回说明：
--- - `boolean`：布尔值。
---@param pt vec2_table 参数 `pt`，类型为 `vec2_table`。
---@param camera cc.Camera 参数 `camera`，类型为 `cc.Camera`。
---@param p vec3_table 参数 `p`，类型为 `vec3_table`。
---@return boolean 布尔值。
function TextField:hitTest(pt, camera, p) end

--- 初始化 `ccui.TextField:init` 对应的对象或状态。
---
--- 返回说明：
--- - `boolean`：初始化是否成功。
---@return boolean 初始化是否成功。
function TextField:init() end

--- 获取 `ccui.TextField:getVirtualRendererSize` 对应的值。
---
--- 返回说明：
--- - `size_table`：获取到的 Lua 表数据。
---@return size_table 获取到的 Lua 表数据。
function TextField:getVirtualRendererSize() end

--- 调用 `ccui.TextField:TextField`。
---
--- 返回说明：
--- - `self`：当前对象，便于链式调用。
---@return self 当前对象，便于链式调用。
function TextField:TextField() end
